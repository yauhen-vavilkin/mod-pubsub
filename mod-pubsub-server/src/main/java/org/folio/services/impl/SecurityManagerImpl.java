    ...
    return doRequest(params, USERS_URL + query, HttpMethod.GET, null)
      .compose(response -> {
        if (response.getCode() == HttpStatus.HTTP_OK.toInt()) {
          ...
          if (users.size() > 0) {
            return Future.succeededFuture(users.getJsonObject(0).mapTo(User.class));
          } else {
      ...
      .compose(response -> {
        if (response.getCode() == HTTP_NO_CONTENT.toInt()) {
          LOGGER.info("The {} user [{}] has been updated", systemUserConfig.getName(),
            updatedUser.getId());
          return Future.succeededFuture(updatedUser);
        } else {
          LOGGER.error("Unable to update the {} user [{}]", systemUserConfig.getName(),
            response.getBody());
          return Future.failedFuture(format("Unable to update the %s user: %s", systemUserConfig.getName(),
            response.getBody()));
        }
      });
  }
